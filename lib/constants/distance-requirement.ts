export const TARGET_PARTNER_SOURCE_GOOGLE_MAPS = "google_maps";

export const DISTANCE_REQUIREMENT_HINT =
  "When enabled, candidates are searched via Google Maps using your coordinates and max distance. Saving with this option on sets the search source to Google Maps.";

export const DISTANCE_REQUIREMENT_VIEW_FOOTER =
  "When distance requirement is enabled, candidates are searched via Google Maps using your coordinates and max distance.";
