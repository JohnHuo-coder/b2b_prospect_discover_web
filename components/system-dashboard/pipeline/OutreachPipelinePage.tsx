...
import { fetchOutreachWorkflow } from "@/lib/api/system-dashboard-client";
import {
  applyOutreachStageCountsToNodes,
  mapOutreachWorkflowStageCounts,
  outreachPipelineInitialEdges,
  outreachPipelineInitialNodes,
  type OutreachNodeData,
} from "@/lib/system-dashboard/outreach-pipeline-flow";
import { OutreachPipelineFlowNode } from "./OutreachPipelineFlowNode";
import "./workflow-canvas.css";

const nodeTypes: NodeTypes = {
  outreachPipeline: OutreachPipelineFlowNode,
};
...
export function OutreachPipelinePage() {
  ...
  const [edges, , onEdgesChange] = useEdgesState(outreachPipelineInitialEdges);
  ...
  const onNodeClick = useCallback(
    (_event: React.MouseEvent, node: Node) => {
      if (node.type !== "outreachPipeline") return;
      const data = node.data as OutreachNodeData;
      if (data.kind !== "step" || !data.finalStage) return;
      router.push(`/system-dashboard/outreach/pipeline/${node.id}`);
    },
    [router]
  );
    ...
    <div className="workflow-canvas flex h-[calc(100vh-3rem)] flex-col bg-zinc-100 px-6 py-6 text-zinc-950">
      ...
      <div className="min-h-0 flex-1 overflow-hidden rounded-xl border border-slate-700/40 shadow-sm">
        ...
        >
          ...
          <MiniMap
            nodeColor={(node) => {
              const kind = (node.data as OutreachNodeData).kind;
