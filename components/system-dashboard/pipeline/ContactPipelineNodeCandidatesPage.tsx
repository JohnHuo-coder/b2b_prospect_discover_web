...
import {
  fetchContactStageDetail,
  type ContactStageDetailCandidate,
} from "@/lib/api/system-dashboard-client";
import type { ContactNodeMeta } from "@/lib/system-dashboard/contact-pipeline-flow";
import { DataTableSection } from "../MetricCard";

const PAGE_SIZE = 25;

export function ContactPipelineNodeCandidatesPage({
  node,
}: {
  node: ContactNodeMeta;
}) {
  const [page, setPage] = useState(1);
  const [candidates, setCandidates] = useState<ContactStageDetailCandidate[]>(
    []
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const result = await fetchContactStageDetail(node.finalStage!);
        if (cancelled) return;
    ...
    <div className="px-6 py-6">
      ...
      {!node.finalStage ? (
        <div className="rounded-xl border border-gray-200 bg-white px-6 py-10 text-center text-sm text-gray-500 shadow-sm">
          This node is an introduction point, not a workflow stage.
        </div>
      ) : (
        <>
          {error ? (
