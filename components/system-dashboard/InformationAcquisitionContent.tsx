...
import {
  fetchInfoAcquisitionCandidateDetail,
  fetchInfoAcquisitionCandidates,
  type AcquisitionCandidate,
  type InfoAcquisitionCandidate,
  type InfoAcquisitionStatusFilter,
} from "@/lib/api/system-dashboard-client";
import { AcquisitionCandidateDetail } from "./AcquisitionCandidateDetail";
import { InformationAcquisitionSummaryCards } from "./InformationAcquisitionSummaryCards";
...
const statusFilters: InfoAcquisitionStatusFilter[] = [
  "all",
  "succeed",
  "failed",
];

const statusFilterLabels: Record<InfoAcquisitionStatusFilter, string> = {
  all: "All",
  succeed: "Succeed",
  failed: "Failed",
};
...
export function InformationAcquisitionContent() {
  ...
  const [statusFilter, setStatusFilter] =
    useState<InfoAcquisitionStatusFilter>("all");
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [selectedDetail, setSelectedDetail] = useState<AcquisitionCandidate | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const result = await fetchInfoAcquisitionCandidates({
  ...
  return (
    <div className="px-8 py-8">
      <SystemDashboardBackLink
        ...
        subtitle="Website scraping results for evaluated candidates"
      />
      ...
      <InformationAcquisitionSummaryCards />
      ...
      <DataTableSection
        ...
        hint="Click a row to view requirement details"
      >
