...
export function IndustryMultiSelect({
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-[44px] flex-wrap items-center gap-2 rounded-lg border border-zinc-200 px-3 py-2 focus-within:border-teal-300 focus-within:ring-2 focus-within:ring-teal-100">
        {value.map((item) => (
          <span
            key={item.id}
        ...
        <input
          ...
          className="min-w-[160px] flex-1 border-0 bg-transparent py-1 text-sm text-zinc-950 outline-none placeholder:text-zinc-400"
        />
      </div>
            ...
            options.map((option) => (
              <li key={option.id} role="option" aria-selected={false}>
                <button
                  ...
                  className="block w-full px-3 py-2 text-left text-sm text-zinc-950 transition hover:bg-teal-50"
                >
                  {option.label}
                </button>
              </li>
