...
function FactsList({ facts }: { facts: unknown }) {
  ...
  if (items.length === 0) {
    return <p className="text-sm text-zinc-500">No facts available.</p>;
  }
...
export function ComplianceCheckReviewContent({
  ...
  if (!detail && !loading) {
    return (
      <p className="text-sm text-zinc-500">No compliance review data available.</p>
    );
  }
  ...
  return (
    <>
      <section className="rounded-xl border border-zinc-200 bg-zinc-50/60 p-4">
        ...
        {loading ? (
          <p className="text-sm text-zinc-500">Loading review details...</p>
        ) : detail ? (
          <dl className="space-y-4">
            ...
            <DetailField label="Issues">
              {issues.length === 0 ? (
                <span className="text-zinc-500">No issues listed.</span>
              ) : (
                ...
                {isEditingEmail ? (
                  <div className="space-y-3">
                    <textarea
                      ...
                      className="max-h-56 w-full resize-y rounded-lg border border-teal-200 bg-white px-3 py-2.5 text-sm leading-relaxed text-zinc-950 outline-none transition focus:border-teal-300 focus:ring-2 focus:ring-teal-100 [overflow-wrap:anywhere]"
                    />
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className="rounded-lg bg-teal-800 px-3 py-1.5 text-sm font-medium text-white transition hover:bg-teal-900 disabled:cursor-not-allowed disabled:opacity-50"
                      >
                        Save
                      </button>
                    </div>
                  ...
                  <div className="max-h-56 min-h-32 overflow-y-auto rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm leading-relaxed text-zinc-950 [overflow-wrap:anywhere] whitespace-pre-wrap">
                    {outreachEmail || (
                      <span className="text-zinc-400">No email text available.</span>
                    )}
                  </div>
                )}
              </dd>
      ...
      <section className="mt-6">
        <h3 className="text-sm font-semibold text-zinc-950">Company facts</h3>
        ...
        <div className="mt-4">
          {loading ? (
            <p className="text-sm text-zinc-500">Loading facts...</p>
          ) : !detail || detail.facts.length === 0 ? (
            <p className="text-sm text-zinc-500">No facts found for this company.</p>
          ) : selectedRequirement === "all" ? (
            <div className="space-y-3">
              {detail.facts.map((row) => (
                <div
                ...
                >
                  ...
                  {row.requirement ? (
                    <p className="mt-2 text-sm text-zinc-600 [overflow-wrap:anywhere]">
                      {row.requirement}
                    </p>
                  ) : null}
                  <div className="mt-3">
            ...
            <div className="rounded-lg border border-zinc-200 bg-white p-4">
              {requirementLoading ? (
                <p className="text-sm text-zinc-500">Loading requirement facts...</p>
              ) : requirementError ? (
                ...
                <p className="text-sm text-zinc-500">No facts available.</p>
              )}
            </div>
          )}
        </div>
