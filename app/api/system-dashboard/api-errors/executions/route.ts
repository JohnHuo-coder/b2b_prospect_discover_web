...
export const GET = withAuth(
  withApproved(async (request: Request, _context: unknown, user: DbUserWithConfig) => {
    try {
      ...
      const apiName = searchParams.get("api_name")?.trim();
      ...
      if (!apiName) {
        return errorResponse("api_name is required", 400);
      }

      const result = await apiErrorRepository.getApiErrorExecutions({
      ...
      console.error("[GET /api/system-dashboard/api-errors/executions]", error);
